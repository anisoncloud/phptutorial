...
{
  $app_info = file_get_contents('http://localhost/phptutorial/api/api.php?action=get_app&id=' . $_GET["id"]);
...
{
  $app_list = file_get_contents('http://localhost/phptutorial/api/api.php?action=get_app_list');
  ...
  ?>
    ...
    <?php foreach ($app_list as $app): ?>
      <li>
        <a href=<?php echo "http://localhost/phptutorial/api/index.php?action=get_app&id=" . $app["id"]  ?> alt=<?php echo "app_" . $app["id"] ?>><?php echo $app["name"] ?></a>
    </li>
    <?php endforeach; ?>
    </ul>
  <?php
